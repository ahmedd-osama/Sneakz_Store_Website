...
let userCart = {
  ...
  getTotalPrice:()=>{
    ...
    userCart.getExtraExpenses()?userCart.getExtraExpenses()?.forEach(elem => totalPrice = totalPrice + elem.value):'';
    return totalPrice;
  },
  getSubTotalPrice:()=>{
    let subtotalPrice = 0;
...
// uppending and updating items section
  ...
  totalPriceEls.forEach((el: any) =>{el.innerHTML = userCart.getTotalPrice()});
  subtotalPriceEls.forEach((el: any) =>{el.innerHTML = userCart.getSubTotalPrice()});
  //Extra Invoice Items
  invoiceDetails.querySelectorAll('.extra-expense').forEach((item:any)=>item.remove())
  userCart.getExtraExpenses()? userCart.getExtraExpenses()?.forEach((expense) =>{
...
select('.next-sec-btn', true)?.forEach((btn: any)=>{
...
select('.prev-sec-btn', true)?.forEach((btn: any)=>{
  btn.addEventListener('click', ()=>{
